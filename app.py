...
# ─── Image Processing (brown-egg tuned) ──────────────────────────────────────
...
def _preprocess_gray(img_bgr: np.ndarray) -> np.ndarray:
    """
    Convert to grayscale and apply CLAHE to boost local contrast.
    Brown eggs often have low contrast against surfaces — CLAHE on gray
...
def _brown_egg_mask(img_bgr: np.ndarray) -> np.ndarray:
    """
    HSV colour mask tuned for Thai brown eggs.
    Brown shell: H 5-25, S 30-160, V 60-220
    Returns a binary mask (255 = likely egg shell pixel).
    """
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)
    # Primary brown range
    lo1 = np.array([5,  30,  60], dtype=np.uint8)
    hi1 = np.array([25, 160, 220], dtype=np.uint8)
    # Darker brown / shadowed
    lo2 = np.array([3,  20,  30], dtype=np.uint8)
    hi2 = np.array([30, 255, 180], dtype=np.uint8)
...
def _edge_map(gray_cl: np.ndarray) -> np.ndarray:
    """
    Four-way edge fusion on the CLAHE-gray image.
    Approach A: light Gaussian  → low-threshold Canny  (catches faint edges)
    Approach B: bilateral filter → Canny               (sharp edges, less noise)
    Approach C: adaptive threshold                      (works when contrast is uniform)
    Approach D: morphological gradient                  (ring-shaped edge around blob)
    """
    # A
    bA = cv2.GaussianBlur(gray_cl, (7, 7), 0)
    eA = cv2.Canny(bA, 10, 40)

    # B
    bB = cv2.bilateralFilter(gray_cl, 11, 80, 80)
    eB = cv2.Canny(bB, 20, 60)

    # C — adaptive threshold (mean of 51×51 neighbourhood)
    eC = cv2.adaptiveThreshold(
        cv2.GaussianBlur(gray_cl, (5, 5), 0),
        255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 51, 6
    )

    # D — morphological gradient
    kern = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    eD   = cv2.subtract(cv2.dilate(gray_cl, kern), cv2.erode(gray_cl, kern))
    _, eD = cv2.threshold(eD, 20, 255, cv2.THRESH_BINARY)

    combined = cv2.bitwise_or(cv2.bitwise_or(eA, eB), cv2.bitwise_or(eC, eD))

    # Close gaps so contour loops are sealed
...
def _score_ellipse(cnt, h: int, w: int) -> float:
    """Return a score for how egg-like a contour is (higher = better). -1 = reject."""
    if len(cnt) < 5:
        return -1
    area = cv2.contourArea(cnt)
    if area < h * w * 0.004 or area > h * w * 0.88:
        return -1
    ...
    if mi < 1:
        return -1
    ratio = MA / mi
    if ratio < 1.05 or ratio > 3.2:
        return -1
    # Penalise if centre is too close to image border
    bm = 0.04
    border_pen = 0.3 if (cx < w*bm or cx > w*(1-bm) or
                         cy < h*bm or cy > h*(1-bm)) else 1.0
    # Reward aspect ratios in the egg-like sweet spot 1.2–1.9
    shape_bonus = 1.3 if 1.2 <= ratio <= 1.9 else 1.0
    return area * border_pen * shape_bonus


def detect_egg_ellipse(img_bgr: np.ndarray):
    """
    Robust egg-ellipse detection for Thai brown eggs.
    1. Grayscale + CLAHE (primary channel for edge detection)
    2. Brown HSV mask (adds contours from colour segmentation)
    3. Four-way edge fusion on gray
    4. Best ellipse chosen by aspect-ratio scoring
    Returns OpenCV ellipse tuple or None.
    """
    h, w   = img_bgr.shape[:2]
    gray_cl = _preprocess_gray(img_bgr)

    # --- Channel 1: edge-based contours ---
    edges    = _edge_map(gray_cl)
    cnts_edge, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # --- Channel 2: brown colour mask contours ---
    brown_mask = _brown_egg_mask(img_bgr)
    cnts_brown, _ = cv2.findContours(brown_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # --- Channel 3: Otsu threshold on gray (handles plain backgrounds) ---
    _, otsu = cv2.threshold(gray_cl, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    ko = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    otsu = cv2.morphologyEx(otsu, cv2.MORPH_CLOSE, ko, iterations=3)
    cnts_otsu, _ = cv2.findContours(otsu, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    all_cnts = list(cnts_edge) + list(cnts_brown) + list(cnts_otsu)

    best, best_score = None, 0
    for cnt in all_cnts:
        score = _score_ellipse(cnt, h, w)
        if score > best_score:
            best_score = score
            best = cv2.fitEllipse(cnt)
...
def _hough_circles_multi(gray: np.ndarray, h: int, w: int):
    """
    Run Hough circle detection with multiple parameter sets on both the raw
    gray image and a CLAHE-enhanced version, then merge results.
    """
    clahe   = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(8, 8))
    gray_cl = clahe.apply(gray)

    blur    = cv2.GaussianBlur(gray,    (9, 9), 2)
    blur_cl = cv2.GaussianBlur(gray_cl, (9, 9), 2)
    # sharper blur for clear metallic edges
    blur_s  = cv2.GaussianBlur(gray,    (5, 5), 1)

    all_circles = []
    param_sets = [
        (blur,    1.2, 60, 30, 0.020, 0.30),
        (blur,    1.5, 50, 25, 0.018, 0.32),
        (blur,    1.0, 80, 40, 0.025, 0.26),
        (blur_cl, 1.2, 55, 28, 0.020, 0.30),
        (blur_cl, 1.5, 45, 22, 0.018, 0.32),
        (blur_s,  1.2, 70, 35, 0.022, 0.28),
    ]
    for img_src, dp, p1, p2, mnf, mxf in param_sets:
        circles = cv2.HoughCircles(
            img_src, cv2.HOUGH_GRADIENT, dp=dp,
            minDist=min(h, w) * 0.06,
            param1=p1, param2=p2,
...
def _classify_coin_by_color(img_bgr: np.ndarray, cx: int, cy: int, r: int) -> tuple:
    """
    Thai coin identification by HSV ring analysis.
    10 Baht — bi-metallic: brass/gold outer ring → H 10-40, S > 55
     5 Baht — silver nickel                      → low saturation, high V
     1 Baht — smaller silver nickel              → low saturation, high V (smaller)
    """
    h_img, w_img = img_bgr.shape[:2]
    # Outer ring: 65–100 % of radius
    mask_out = np.zeros((h_img, w_img), dtype=np.uint8)
    mask_in  = np.zeros((h_img, w_img), dtype=np.uint8)
    cv2.circle(mask_out, (cx, cy), r,           255, -1)
    cv2.circle(mask_in,  (cx, cy), int(r*0.65), 255, -1)
    ring_mask = cv2.subtract(mask_out, mask_in)

    hsv  = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)
    ring = hsv[ring_mask > 0]
    if len(ring) == 0:
        return "5 Baht (24 mm)", 0.4, "silver"

    H, S, V = ring[:, 0], ring[:, 1], ring[:, 2]
    gold_px   = ((H >= 10) & (H <= 40) & (S > 55) & (V > 80)).sum()
    copper_px = (((H <= 10) | (H >= 160)) & (S > 55) & (V > 60)).sum()
    total     = len(ring)

    gold_r   = gold_px   / total
    copper_r = copper_px / total

    if gold_r > 0.10:
        return "10 Baht (26 mm)", round(min(gold_r * 5, 1.0), 2), "gold"
    if copper_r > 0.07:
        return "5 Baht (24 mm)", round(min(copper_r * 6, 0.85), 2), "silver"

    # Fallback: relative size
...
def detect_coin(img_bgr: np.ndarray):
    """Detect + classify coin. Returns (circle_arr, type_str, conf, colour_tag)."""
    h, w   = img_bgr.shape[:2]
    gray   = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
...
def detect_coin_manual(img_bgr: np.ndarray):
    """Detect largest circle only (user supplies the type)."""
    h, w   = img_bgr.shape[:2]
    gray   = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    circles = _hough_circles_multi(gray, h, w)
    if not circles:
