...
def compute_egg_metrics(a: float, b_max: float, size: int) -> dict:
    ...
    # Volume
    # Note: the original Python source had a typo in the first term —
    # `(1/(9-6ka)) + (1/(9-6ka))` (same expression twice). By symmetry with
    # the second term `(1/(9-3ka)) + (1/(9+3ka))` it should be a +/- mirror.
    # Without this fix the volume comes out ~30 % too low.
    V = (
        ((2 / 9) * math.pi * a * (b ** 2))
        * (
...
def load_yolo_models():
    ...
    Also detects common deployment problems (missing files, Git LFS pointer
    stubs, wrong filename) and surfaces them in the UI."""
    if not _ULTRALYTICS_AVAILABLE:
        return None, None, "ultralytics not installed"

    egg_m, coin_m, msgs = None, None, []

    def _diagnose(path: str, label: str):
        """Return None if file is OK, else a diagnostic string."""
        if not os.path.exists(path):
            return f"{label}: file not found at `{path}` (working dir: `{os.getcwd()}`)"
        size = os.path.getsize(path)
        if size < 200:                       # real .pt files are MB-sized
            # Probably a Git LFS pointer stub — check first bytes
            try:
                with open(path, "rb") as f:
                    head = f.read(100)
                if b"version https://git-lfs" in head:
                    return (f"{label}: file is a **Git LFS pointer**, not the real model "
                            f"({size} bytes). Re-upload the actual `.pt` file without LFS, "
                            f"or enable LFS pulls on Streamlit Cloud.")
            except Exception:
                pass
            return f"{label}: file too small ({size} bytes) — corrupt or not a model"
        return None

    # Egg model
    err = _diagnose(EGG_MODEL_PATH, "Egg model")
    if err:
        msgs.append(err)
    else:
        try:
            ...
            msgs.append(f"Egg model load failed: {e}")

    # Coin model
    err = _diagnose(COIN_MODEL_PATH, "Coin model")
    if err:
        msgs.append(err)
    else:
        try:
            ...
            msgs.append(f"Coin model load failed: {e}")

    return egg_m, coin_m, " · ".join(msgs)
...
def show_results(r: dict, size: int, a_input: float, b_max_input: float):
    ...
    with col1:
        st.metric("📏 Length",      f"{length_disp:.2f} mm")
    ...
    with col2:
        st.metric("📐 Width",       f"{width_disp:.2f} mm")
